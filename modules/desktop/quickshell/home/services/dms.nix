...
let
  ...
  runtimePath = lib.makeBinPath (
    [ dmsPackage ] ++ lib.optional (quickshellPackage != null) quickshellPackage
  );
in
{
  options.gb.home.desktop.mango.services.dms.enable = lib.mkEnableOption "Enable DMS startup service";

  config =
    lib.mkIf (mango.enable && mango.dms.enable && mango.services.enable && mango.services.dms.enable)
      {
